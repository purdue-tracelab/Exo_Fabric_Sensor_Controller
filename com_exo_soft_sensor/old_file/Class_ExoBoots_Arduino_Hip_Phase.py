...
class ExoBoot:
    ...
    def __init__(self, port, baud_rate, idx, log_level, shouldLog, frequency, soft_sensor_data):
        ...
        self.idx_toeoff_gait = self.idx_other_base + 7
        ...
        self.labels_other = [
            ...
            "Torque Command", "Hip Soft Sensor", "ToeOff Percent Gait", "MHE Percent Gait"]
    ...
    def read_data(self, sensor_data):
        ...
        if self.data_current[self.idx_time] != self.data_exo[self.idx_time]:  # update the data if new data has come in
            ...
            # self.data_other[self.idx_soft_sensor - self.idx_other_base] = sensor_data
            self.data_other[self.idx_toeoff_gait - self.idx_other_base] = (1 if self.toeoff_segment_trigger else 0)
            # self.data_other[self.idx_toeoff_gait - self.idx_other_base] = self.mhe_percent_gait_previous
    ...
    def find_mhe(self):
        ...
        if self.mhe_current == -1:
            self.mhe_current = self.data_other[self.idx_soft_sensor - self.idx_other_base]
            return

        mhe_temp = self.data_other[self.idx_soft_sensor - self.idx_other_base]
        # if (self.percent_gait >= 20) and (mhe_temp < self.mhe_current):
        if (self.percent_gait >= 20) and (self.percent_gait <= 59)and (mhe_temp < self.mhe_current):
    ...
    def find_mhf(self):
        ...
        mhf_temp = self.data_other[self.idx_idx_soft_sensor - self.idx_other_base]
    ...
    def update_expected_toeoff_gait(self):
        ...
        elif (self.toeoff_percent_gait <= 1.2 * max(self.past_toeoff_times)) and (self.toeoff_percent_gait >= 0.75 * min(self.past_toeoff_times)):
            self.past_toeoff_times.insert(0, self.toeoff_percent_gait)  # insert the new value at the beginning
            self.past_toeoff_times.pop()  # remove the last value
    ...
    def update_expected_mhe(self):
        ...
        elif (self.mhe_percent_gait_previous <= 1.5 * max(self.past_mhe)) and (self.mhe_percent_gait_previous >= 0.5 * min(self.past_mhe)):
            self.past_mhe.insert(0, self.mhe_percent_gait_previous)  # insert the new value at the beginning
            self.past_mhe.pop()  # remove the last value
...
class Arduino:
    ...
    def obtain_softsensor_data(self):
        ...
        try:
            ...
            # print(dataarray)
            self.rawdata1 = float(dataarray[1])  # raw value of the textile sensor 1, usually left hip
            self.rawdata2 = float(dataarray[2])  # raw value of the textile sensor 2, usually left knee
            self.rawdata3 = float(dataarray[3])  # raw value of the textile sensor 3, usually right hip
            self.rawdata4 = float(dataarray[4])  # raw value of the textile sensor 4, usually right knee
