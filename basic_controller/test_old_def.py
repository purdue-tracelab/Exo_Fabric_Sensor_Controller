...
CURRENT_LIMIT = 30000 # mA : Current limit for the motor command.  You should still set the values in boots protection circuit because the PI controller and position controller can still push it higher.
...
class ExoBoot:
	...
	#
	def __init__ (self, devSide, port, baudRate, idx, frequency = 1000,  shouldLog = True, shouldAuto = 1):
		...
		if not fxStartStreaming(self.devId,self.frequency,self.should_log,self.should_auto): # start streaming
		...
		self.kt = 56 		# motor torque constant Nmm/A 56 is the torque constant given by Dephy.
